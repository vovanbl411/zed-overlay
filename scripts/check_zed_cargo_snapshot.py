...
LOCAL_ASSIGNMENT = re.compile(r'^\s*local\s+([A-Z][A-Z0-9_]*)="((?:[^"\\]|\\.)*)"\s*$')
GIT_CRATE_COMMIT = re.compile(r"^\s*git_crate_commit\s+([A-Z][A-Z0-9_]*)\s+([A-Za-z0-9_-]+)\s*$")
...
def parse_offline_substitutions(
    ...
    """Validate explicit offline replacements sourced directly from GIT_CRATES."""
    variables: dict[str, str] = {}
    commit_sources: dict[str, str] = {}
    for line in ebuild_text.splitlines():
        assignment = LOCAL_ASSIGNMENT.fullmatch(line)
        if assignment is not None:
            ...
            continue
        extraction = GIT_CRATE_COMMIT.fullmatch(line)
        if extraction is not None:
            variable, crate = extraction.groups()
            if variable in commit_sources:
                raise CargoSnapshotError(f"Duplicate GIT_CRATES commit extraction for {variable!r}.")
            if crate not in git_crates:
                raise CargoSnapshotError(
                    f"GIT_CRATES commit extraction for {variable!r} references missing crate {crate!r}."
                )
            commit_sources[variable] = crate

    for variable, crate in commit_sources.items():
        variables[variable] = git_crates[crate].commit
    ...
    for name, raw_declaration in variables.items():
        ...
        commit_variable = f"{stem}_COMMIT" if not commit_variables else commit_variables[0]
        if len(commit_variables) > 1 or not commit_variable.endswith("_COMMIT"):
            raise CargoSnapshotError(f"Offline Git declaration {name!r} has no single commit variable.")
        if commit_variable not in commit_sources:
            raise CargoSnapshotError(
                f"Offline Git declaration {name!r} is not sourced from GIT_CRATES."
            )
        ...
        crate, _repository, declared_commit = match.groups()
        commit = variables[commit_variable]
        if declared_commit is not None and declared_commit != commit:
            raise CargoSnapshotError(f"Offline Git declaration {name!r} disagrees with GIT_CRATES.")
        ...
        substitutions.append(OfflineSubstitution(crate, declaration))
