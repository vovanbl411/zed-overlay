...
SCRIPT_PATH = REPOSITORY_ROOT / "scripts" / "check_zed_cargo_snapshot.py"
...
\tgit_crate_commit EXAMPLE_COMMIT example
...
class CargoSnapshotTests(unittest.TestCase):
    ...
    def test_accepts_matching_snapshot_and_git_crates_sourced_substitution(self) -> None:
        self.validate()
...
'''
    ...
    def test_rejects_repository_mismatch(self) -> None:
        lock = self.source / "Cargo.lock"
        lock.write_text(
            lock.read_text(encoding="utf-8").replace(REPOSITORY, OTHER_REPOSITORY), encoding="utf-8"
        )

        with self.assertRaisesRegex(CHECKER.CargoSnapshotError, "repository differs"):
            self.validate()

    def test_rejects_offline_substitution_not_sourced_from_git_crates(self) -> None:
        ebuild = self.ebuild.read_text(encoding="utf-8")
        self.ebuild.write_text(
            ebuild.replace("\tgit_crate_commit EXAMPLE_COMMIT example\n", ""), encoding="utf-8"
        )

        with self.assertRaisesRegex(CHECKER.CargoSnapshotError, "not sourced from GIT_CRATES"):
            self.validate()

    def test_rejects_missing_git_crates_commit_source(self) -> None:
        ebuild = self.ebuild.read_text(encoding="utf-8")
        self.ebuild.write_text(
            ebuild.replace("git_crate_commit EXAMPLE_COMMIT example", "git_crate_commit EXAMPLE_COMMIT missing"),
            encoding="utf-8",
        )

        with self.assertRaisesRegex(CHECKER.CargoSnapshotError, "references missing crate"):
            self.validate()
    ...
    def test_current_ebuild_extracts_all_offline_commits_from_git_crates(self) -> None:
        ebuild_text = (REPOSITORY_ROOT / "app-editors/zed/zed-1.21.0.ebuild").read_text(
            encoding="utf-8"
        )
        crates = CHECKER.parse_git_crates(ebuild_text)
        substitutions = CHECKER.parse_offline_substitutions(ebuild_text, crates)

        self.assertEqual(
            {substitution.crate for substitution in substitutions},
            {
                "async-process",
                "async-task",
                "calloop",
        ...
        self.assertTrue(all(substitution.declaration for substitution in substitutions))
