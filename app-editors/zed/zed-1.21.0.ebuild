...
src_prepare() {
	...
	git_crate_commit() {
		...
		local value="${GIT_CRATES[${crate}]}"
		local repository remainder commit unpack_path

		[[ -n "${crate}" && -n "${output_variable}" && -n "${value}" ]] \
			|| die "Missing GIT_CRATES entry for ${crate}"
		repository="${value%%;*}"
		remainder="${value#*;}"
		commit="${remainder%%;*}"
		unpack_path="${remainder#*;}"
		[[ -n "${repository}" && "${remainder}" != "${value}" \
			&& -n "${commit}" && "${unpack_path}" != "${remainder}" \
			&& -n "${unpack_path}" && "${unpack_path}" != *";"* \
			&& "${commit}" =~ ^[0-9a-f]{7,64}$ ]] \
			|| die "Malformed GIT_CRATES entry for ${crate}"
		printf -v "${output_variable}" '%s' "${commit}"
	}

	local ASYNC_PROCESS_COMMIT
	git_crate_commit async-process ASYNC_PROCESS_COMMIT
	...
	local ASYNC_TASK_COMMIT
	git_crate_commit async-task ASYNC_TASK_COMMIT
	...
	local CALLOOP_COMMIT
	git_crate_commit calloop CALLOOP_COMMIT
	...
	local LIVEKIT_COMMIT
	git_crate_commit livekit LIVEKIT_COMMIT
	...
	local NOTIFY_COMMIT
	git_crate_commit notify NOTIFY_COMMIT
	...
	local WIN_CAP_COMMIT
	git_crate_commit windows-capture WIN_CAP_COMMIT
	...
	local TREE_SITTER_COMMIT
	git_crate_commit tree-sitter-language TREE_SITTER_COMMIT
